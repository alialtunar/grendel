...
_STYLES_REQUIRING_KEY = {"openai", "anthropic"}
...
class HTTPTargetAdapter(TargetAdapter):
    ...
    async def send(self, request: AdapterRequest) -> AdapterResponse:
        ...
        if style in _STYLES_REQUIRING_KEY and not self.api_key:
            raise ProviderError(
    ...
    def _build_request(
        ...
        if style == "anthropic":
            ...
            return f"{self.base_url}/messages", body, headers
        ...
        if style == "ollama":
            ...
            return f"{self.base_url}/api/chat", body, headers
